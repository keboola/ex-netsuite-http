...
It supports five extraction modes:
...
- **Metadata** — dump the account's schema catalog: every record type and its field definitions, written to two fixed tables (`record_types` and `fields`). Takes no other row settings; it is always a full-load snapshot.
